...
```html title="index.html" caption="Adding steer buttons" showLineNumbers{9}
<p>Steer Ships:</p>
        <div>
            <button id="steer0">Steer 0</button>
            <button id="steer1">Steer 1</button> <!-- [!code ++] -->
            <button id="steer2">Steer 2</button> <!-- [!code ++] -->
        </div>
</fieldset>
```
...
```html title="index.html" caption="Adding loop checkboxes" showLineNumbers{10}
    <p>Steer Ships:</p>
    <div>
        <button id="steer0">Steer 0</button>
    ...
    <p>Loop Ships:</p> <!-- [!code ++] -->
    <div> <!-- [!code ++] -->
        <input type="checkbox" id="loop0" checked>Loop 0</input> <!-- [!code ++] -->
        <input type="checkbox" id="loop1" checked>Loop 1</input> <!-- [!code ++] -->
        <input type="checkbox" id="loop2" checked>Loop 2</input> <!-- [!code ++] -->
    </div> <!-- [!code ++] -->
</fieldset>
```
...
```js title="solve.js" caption="Implementing looped rotations" showLineNumbers{40}
window.lastRot = 0; // [!code ++]
...
socket.onmessage = function (event) {
    ...
    // Guard clause for looping ships! // [!code ++]
    if (performance.now() - window.lastRot < 500) return; // [!code ++]
    window.lastRot = performance.now(); // [!code ++]
    // Sends steer if checkbox is checked // [!code ++]
    findAll("loop").forEach(function (element, index) { // [!code ++]
        if (element.checked) { // [!code ++]
            socket.send(JSON.stringify({ // [!code ++]
                type: "SHIP_STEER", // [!code ++]
                shipId: `${index}` // [!code ++]
            })); // [!code ++]
...
```js title="solve.js" caption="Creating check() function" showLineNumbers{70} skip{78-151}
let hasRotated = {
    0: false,
    1: false,
    2: false,
...
```html title="index.html" caption="Adding lane checkboxes" showLineNumbers{21}
            <p>Loop Ships:</p>
            <div>
            ...
            <p> Disable Obstacles: </p> <!-- [!code ++] -->
            <div> <!-- [!code ++] -->
                <input type="checkbox" id="toggle0">1/2</input> <!-- [!code ++] -->
                <input type="checkbox" id="toggle1">3/4</input> <!-- [!code ++] -->
                <input type="checkbox" id="toggle2">5/8</input> <!-- [!code ++] -->
                <input type="checkbox" id="toggle3">6/10</input> <!-- [!code ++] -->
                <input type="checkbox" id="toggle4">7/12</input> <!-- [!code ++] -->
                <input type="checkbox" id="toggle5">9/11</input> <!-- [!code ++] -->
            </div>
        </fieldset>
```

```js title="solve.js" caption="Toggleable lanes" showLineNumbers{80}
let checkList = findAll("toggle"); // [!code ++]
...
```js title="solve.js" caption="Implementing auto-dock" showLineNumbers{65} skip{70-116,121-202}
    new Obstacle(1070, 350), //10
    new Obstacle(1530, 200), //11
    new Obstacle(1730, 300), //12
...
function check90(s, o, d) { // [!code ++]
    // Calculates middle of ship in coordinates // [!code ++]
    let mid = Math.abs(Math.floor(s.topLeft.x + s.bottomRight.x) / 2); // [!code ++]
    if (!hasRotated[s.id] && // [!code ++]
        Math.abs(s.topLeft.y - o.y) < 400 && // Large y for legroom // [!code ++]
        Math.abs(mid - o.x) < 20 && // Small x for accuracy // [!code ++]
        s.direction == d) { // [!code ++]
        hasRotated[s.id] = true; // [!code ++]
        socket.send(JSON.stringify({ // [!code ++]
            type: "SHIP_STEER", // [!code ++]
            shipId: s.id // [!code ++]
        })); // [!code ++]
        setTimeout(() => { // [!code ++]
            hasRotated[s.id] = false // [!code ++]
        }, "1000"); // [!code ++]
    } // [!code ++]
} // [!code ++]
```
