...
![Genshin Impact Pity](genshin-pity.svg)
...
![Project Sekai Gacha](project-sekai-gacha.svg)
...
![SekaiCTF 2022 Pie Chart](2022-pie-chart.svg)
...
![Brainstorming I](brainstorming-1.svg)
...
![Brainstorming II](brainstorming-2.svg)
...
```ts title="__tests__/main.test.ts" showLineNumbers
...
>
  ...
  [fontworks.co.jp](https://fontworks.co.jp) > **Translation**: I checked out
  the fonts available for the new Custom Profile feature added in the 1.9.0
  update of
  [#Proseka](https://twitter.com/hashtag/%E3%83%97%E3%83%AD%E3%82%BB%E3%82%AB).
  There are 10 typefaces in total, all from Fontworks. >
...
![Figma Project](figma-project.png)
...
![Unity Hierarchy](unity-hierarchy.png)
...
```cs title="Assets/Scripts/GachaManager.cs" caption="Suspicious base64 string in CreateGachaWebRequest" showLineNumbers{57} {4}
    UnityWebRequest CreateGachaWebRequest(string json)
    {
        byte[] bodyRaw = System.Text.Encoding.UTF8.GetBytes(json);
        string requestUrl = "aHR0cDovLzE3Mi44Ni42NC44OTozMDAwL2dhY2hh";
...
```cs title="Assets/Scripts/GachaManager.cs" caption="Splash art display in DisplaySplashArt" showLineNumbers{38} skip{46-73} {7}
            if (request.result == UnityWebRequest.Result.Success)
            {
                HandleGachaResponse(request.downloadHandler.text, numPulls);
                GachaResponse response = JsonUtility.FromJson<GachaResponse>(
...
| `public void MusicVolume(){:cs}`                       | Used in the volume sliders in `menuModal{:cs}`. Utilizes the `AudioController{:cs}` (which we'll go over later) to adjust sounds: ![Volume Sliders](volume-sliders.png) Also plays a little sound effect which changes pitch depending on the direction the slider is moving. Of course, since we don't want the sound effect to be spammed per tick of the slider, I added a cooldown.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| `public void OnPullButtonClick(){:cs}`                 | **Parameters**: `int cost, int numPulls{:cs}`<br />Opens the `pullConfirmationModal{:cs}` and changes the various TextMeshPro objects inside to be accurate (changes the color of `currentCrystalsText{:cs}` to red and disables `pullConfirmationContinueButton{:cs}` if the user doesn't have enough crystals): ![Confirmation Modal](confirmation-modal.png)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `public void GenericModalHandler(){:cs}`               | **Parameters**: `GameObject modal, Button closeButton{:cs}`<br />Plays the default zoom-in/zoom-out animations, sound effects, and enables darkened background whenever a modal is opened via button: ![Modal Handler](modal-handler.gif)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `private IEnumerator SetInactiveAfterDelay(){:cs}`     | **Parameters**: `GameObject gameObject, float delay{:cs}`<br />Just a simple utility method which is typically called when closing a modal. Since we want the closing animation to play (rather than disabling the modal immediately), we wait a little bit before setting it to inactive.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `private void UpdateSplashArtImage(){:cs}`             | **Parameters**: `string splashArt, Image image{:cs}`<br />Changes the image after each iteration through the splash art carousel, called at the beginning of every splash art coroutine.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `private void DisplayAvatars(){:cs}`                   | **Parameters**: `Character[] characters{:cs}`<br />Handles the instantiation/destruction of `avatarImages{:cs}` within the Grid Layout Group component of `gachaOverviewCanvas{:cs}`: ![Gacha Overview](gacha-overview.gif)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `private void SetImageOpacity(){:cs}`                  | **Parameters**: `MaskableGraphic image, float targetOpacity{:cs}`<br />Utility method exclusively for `ResetAnimations(){:cs}`. Used to set transparency to 0 when resetting animations.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `private bool CheckForSkipOrClick(){:cs}`              | **Parameters**: `Character[] characters, int index{:cs}`<br />An isolated part of the `DisplaySplashArt(){:cs}` coroutine, which checks to see if the user has clicked on the screen to iterate through the carousel.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
![Sekai.best Asset Viewer](sekai-best-asset-viewer.png)

![PJSekai Database](pjsekai-database.png)
...
![Audio Inspector](audio-inspector.png)

```cs title="Assets/Scripts/UIManager.cs" caption="Audio Controller Usage" showLineNumbers{433} skip{435-444} {14,16}
    private void DisplayGachaOverview()
    {
        if (gameState.crystals < 1000)
        {
...
![Background Scroll](background-scroll.gif)
...
![Shrink Button](shrink-button.gif)
...
![Fullscreen Fix](fullscreen-fix.gif)
...
>
> ![In-game screenshot of gacha banner](initial-launch.png)

</Challenge>

We're initially provided with a `dist/` folder with the following structure:

![Initial Folder Structure](initial-folder-structure.svg)
...
![Initial Launch](initial-launch.png)
...
![Gacha Details](gacha-details.png)
...
![Initial dnSpy](initial-dnspy.png)
...
![Suspicious String](suspicious-string.png)
...
```cs title="Assembly-CSharp.dll" caption="CreateGachaWebRequest Method" showLineNumbers{51} {9-10}
...
```cs title="Assembly-CSharp.dll" caption="SendGachaRequest Method" showLineNumbers{27} {4}
...
![Recompiling DLL](recompiling-dll.png)

```cs title="Assembly-CSharp.dll" caption="Modifying GachaRequest Parameters" showLineNumbers
using System;
using System.Collections;
using RequestClasses;
using UnityEngine;
...
public partial class GachaManager : MonoBehaviour
{
	...
	public IEnumerator SendGachaRequest(int numPulls)
	{
        string json = JsonUtility.ToJson(new GachaRequest(this.gameState.crystals, this.gameState.pulls, numPulls)); // [!code --]
		string json = JsonUtility.ToJson(new GachaRequest(10000000, 999999, numPulls)); // [!code ++]
		using (UnityWebRequest request = this.CreateGachaWebRequest(json))
		{
			yield return request.SendWebRequest();
			if (request.result == UnityWebRequest.Result.Success)
		...
		UnityWebRequest request = null; // [!code --]
		yield break; // [!code --]
		yield break; // [!code --]
	}
}
