...
<details>
  <summary class="cursor-pointer"><b>Complicated Math Symbols</b></summary>
...
[[0;32m+[0m] Opening connection to 0.cloud.chals.io on port <span style="color:#696969">[PORT]</span>: Done
...
```python title="matchmaker.py" caption="Maximum Weight Matching (test case)" showLineNumbers
from pwn import remote # [!code --]
import networkx as nx # [!code ++]
import networkx.algorithms.matching as matching # [!code ++]
...
def main() -> None:
    p = remote('0.cloud.chals.io', [PORT]) # [!code --]
    data = parse_input(p.recvuntilS(b'> ')) # [!code --]
    data = [[0, 0, 100, 0], [0, 100, 0, 0], [0, 100, 0, 0], [100, 0, 0, 0]] # [!code ++]
    G: nx.Graph = nx.Graph([(i, j, {'weight': data[i][j] + data[j][i]}) for i in range(len(data)) for j in range(len(data[i])) if i != j]) # [!code ++]
    M: set[tuple] = matching.max_weight_matching(G, maxcardinality=True) # [!code ++]
    M_S: list[tuple] = sorted(list(M) + [(b, a) for a, b in M]) # [!code ++]
    print(data) # [!code ++]
    print(M_S) # [!code --]
...
```python title="matchmaker.py" caption="Final Script" showLineNumbers
from pwn import remote # [!code ++]
...
def main() -> None:
    ...
    for _ in range(3): # [!code ++]
        ...
        result: str = ';'.join([f'{a},{b}' for a, b in M_S]) # [!code ++]
        print(data) # [!code --]
        p.sendline(result.encode()) # [!code ++]
