...
description: "Ever wanted to guess an integer that's 617 digits long? Well, I did. Here's how I did it using ancient abacus math (Chinese Remainder Theorem)."
date: 2023-07-24
...
![Source Code Graphical Visualization](visualization.svg)
...
<InfoBox type="info" title="Recall">
...
![Carnival Game](carnival-visual.svg)
...
<InfoBox>
**Primorial**: For the $$n$$th prime number $$p_n$$, the primorial $$p_n\#$$ is defined as the product of the first $$n$$ primes:
...
![Bob Handing Papers](handing-papers.svg)
...
```py title="solve.py" caption="v2 Solve Attempt" showLineNumbers
from sympy.ntheory.modular import crt
from sympy import primorial
from tqdm import tqdm
from pwn import *
...
p = remote('amt.rs', 31692) # [!code --]
p = remote('amt.rs', 31693) # [!code ++]

for _ in range(10): # [!code --]
    remainders = []
    moduli = []

    for j in tqdm(range(1412)): # [!code --]
    for j in tqdm(range(16)): # [!code ++]
        n = -j
        m = primorial
        p.sendlineafter(b'n m: ', bytes(f'{n} {m}', 'utf-8'))
...
```py title="solve.py" caption="Changing Primorial" showLineNumbers{6}
# Generate the 256th primorial # [!code --]
N = 2**8 # [!code --]
# Generate the 1024th primorial # [!code ++]
N = 2**10 # [!code ++]
primorial = primorial(N)
```
